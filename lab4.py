...
while not finished:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if (click(event)[0] - event.pos[0])**2 + (click(event)[1] - event.pos[1])**2 <= click(event)[2]**2 or (click2(event)[0] - event.pos[0])**2 + (click2(event)[1] - event.pos[1])**2 <= click(event)[2]**2:
                print('yes')
                ochki = ochki + 1
